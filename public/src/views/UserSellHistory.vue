<template>
  <div class="sell-history">
    <router-view v-if="$route.name === 'EditSellRecord' || $route.name === 'CreateSellRecord'" />
    <el-card v-else>
      <template #header>
      ...
      >
        <el-table-column
          ...
          label="卖出时间"
          width="180"
          :formatter="(row: SellRecord) => formatDate(row.sellTime)"
        />
        <el-table-column
          prop="assets.name"
          label="资产名称"
        ...
        <el-table-column label="币种" width="100">
          <template #default="{ row }: { row: SellRecord }">
        ...
        <el-table-column
          ...
          width="120"
          align="right"
          :formatter="(row: SellRecord) => formatNumber(row.sellPrice, 3)"
        />
        <el-table-column
          prop="amount"
          label="数量"
          width="120"
          align="right"
          :formatter="(row: SellRecord) => formatNumber(row.amount, 4)"
        />
        <el-table-column
          prop="pnl"
          label="盈亏"
...
import { getSellHistory } from '@/api/sells'
import type { SellRecord } from '@/api/sells'
...
const fetchRecords = async () => {
  try {
    loading.value = true
    const response = await getSellHistory({
      userid: userId.value,
...
// 处理编辑
const handleEdit = (row: SellRecord) => {
  // 只保留需要的字段，避免类型问题
  const cleanRecord = {
    id: row.id,
    ...
    buys: row.buys ? {
      id: row.buys.id,
      assetId: row.buys.assetId,
      currencyId: row.buys.currencyId,
      exchangeRate: row.buys.exchangeRate,
      price: row.buys.price,
      amount: row.buys.amount,
      buyTime: row.buys.buyTime,
      assets: row.buys.assets ? {
        id: row.buys.assets.id,
        name: row.buys.assets.name
      } : undefined
    } : undefined
  }
  
  // 将数据存储在 sessionStorage 中
  sessionStorage.setItem('editSellRecord', JSON.stringify(cleanRecord))
...
watch(
  ...
  (newRouteName) => {
    // 当从编辑页面返回到历史页面时，刷新数据
    if (newRouteName === 'UserSellHistory') {
      fetchRecords()
    }
  }
)
